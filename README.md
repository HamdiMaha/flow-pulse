...
| **Results table** | Fixed columns (Date, Status, ID, Category, Severity, Jira) **plus one column per extra header in the sheet**. Filter tabs — All/Passed/Failed/Ignored — recompute against whatever's currently searched (type "MFA" and the tab counts narrow to just that). Free-text search (incl. extra columns, driven by Breakdown clicks too), `Export CSV` of the current filter, incremental "show more". Every flow except AGA and Tiles (see below). |
| **Combinations Summary** (AGA and Tiles only) | Hardcoded to those two buckets (not a general per-bucket setting), and only when the flow actually has the needed columns — AGA: Entrypoint/Line Calculator/Plan/Region; Tiles: User ID (`user_id`)/Tile ID (the `id` column)/Tile Name (`tile_nm`), no screenshots. Tiles also hides its `BUP` and `log_file` columns everywhere. Replaces the Results Table with: 4 cascading dropdown filters (each narrows the next) plus a 5th **Success Rate** dropdown (lists whatever pass rates currently show up in the table — it's a computed value, not a raw column, so it filters the table rather than cascading into anything after it), a "Combinations Summary" table — one row per unique combination of the four columns, with Success Rate — click a row for "Detailed annotations for selected combo" (every column for the matching raw rows, with All/Passed/Failed/Ignored tabs), click a row there for its screenshot if the team uploaded one, matched by the row's `Unnamed: 0` column value (a stray index column from pandas-exported Excel files). Screenshot must sit in the same folder as that day's `.xlsx`. |
